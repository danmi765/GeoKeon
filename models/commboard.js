...
exports.list = (req, res) => {
    console.log('/commboard/list', req.body);
    const reqBody = req.body;
    dbconn.instance[defaultDB].query(queries.select.list_comm_board, [tables['BOARD_INQUIRY'], reqBody], function (error, results, fields) {
        if (error){
...
exports.write = function(req, res){
    const reqBody = req.body;
    dbconn.instance[defaultDB].query(queries.insert.add_comm_board, [tables['BOARD_INQUIRY'], reqBody], function (error, results, fields) {
        if (error){
            return res.send({'error': error});
        }
        console.log('/commboard/list results', results);
